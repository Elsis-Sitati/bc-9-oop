...
class TestNotesApplication(unittest.TestCase):
	...
	def test_if_parameter_is_a_list(self):
        author = 'Rehema Mkazi'
        notes_list = ["I dont think there is a better learning environment than andela"]
        note_application = NotesApplication(author, notes_list)
        self.assertEqual(type(noteApp.notes), list, msg='Second argument should be a list.')
    ...
    def test_if_create_method_is called_with_one_argument(self):
        with self.assertRaises(TypeError):
            note_application = NotesApplication('Ronaldo', ['I am better than Messi ', 'Ozil made me shine'])
